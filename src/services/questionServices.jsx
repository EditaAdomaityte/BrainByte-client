export const getresponses = async (quizId) => {
  const response = await fetch(
    `http://localhost:8000/quizresponses?quizattempt_id=${quizId}`,
    {
      headers: {
        Authorization: `Token ${
...
export const deleteQuestion = async (questionId) => {
  try {
    ...
    if (response.ok) {
      ...
      return {
        success: false,
        error: errorData.reason || "Failed to delete question",
      };
    }
  } catch (error) {
...
export const getQuestionsByCategory=async(categoryId)=>{
    const response=await fetch(`http://localhost:8000/questions?category_id=${categoryId}`,{
        headers:{
            Authorization: `Token ${
                JSON.parse(localStorage.getItem("user_token")).token
              }`,
...
export const deleteCategoryInQuestion = async (questionId) => {
  await fetch(
    `http://localhost:8000/questioncategories/delete_by_question?question_id=${questionId}`,
    {
      method: "DELETE",
      headers: {
        Authorization: `Token ${
...
export const getCategoriesInQuestion = async (questionId) => {
  const response = await fetch(
    `http://localhost:8000/questioncategories?question_id=${questionId}`,
    {
      headers: {
        Authorization: `Token ${
          JSON.parse(localStorage.getItem("user_token")).token
